...
class mysqlUtils():
    ...
    def select_bioproject_by_cra_accession(self,cra):
        sql = "select * from project WHERE project.prj_id = (select prj_id from cra WHERE cra.accession = (%s));" #.format(country.strip().strip("'").strip())
        self.excute(sql,cra)
        return(self.fetchOne())

    def select_sample_scope_name_by_cra_accession(self,cra):
        sql = "select s.sample_scope_name from prj_sample_scope s WHERE s.sample_scope_id = (select p.sample_scope_id from project p WHERE p.prj_id = (select prj_id from cra WHERE cra.accession = (%s))); " #.format(country.strip().strip("'").strip())
        self.excute(sql,cra)
        res = self.fetchOne()
    ...
    def select_biosampleAndExperimentAndRUN_by_cra_accession(self,cra):
        sql = """
            ...
            ) as ser on rdf.run_id = ser.run_run_id where status=10;
        """ 
        self.excute(sql,cra)
        res = self.fetchAll()
        ...
        if res == None :
            ...
            for z in res:
                ...
                if k not in CRR_dict.keys():
                    CRR_dict[k] = { m:n for m,n in z.items() if not str(m).strip().startswith("rdf") }
                    CRR_dict[k]['file_list'] = [
                         { m:n for m,n in z.items() if str(m).strip().startswith("rdf") }
                    ]
                else:
            ...
            for crrid in list(CRR_dict.keys()):
                ...
                # library_name
                CRR_dict[crrid]['formatted_attrs']['library_name'] = "missing" if str(CRR_dict[crrid]['exp_lib_name']) == "None" else str(CRR_dict[crrid]['exp_lib_name'])

                # library_protocol
                CRR_dict[crrid]['formatted_attrs']['library_construction_protocol'] = "missing" if CRR_dict[crrid]['exp_lib_design'] == None else str(CRR_dict[crrid]['exp_lib_design'])


                #library layout
                CRR_dict[crrid]['formatted_attrs']['library_layout'] = "SINGLE" if str(CRR_dict[crrid]['exp_lib_layout']).strip() == "1" else "PAIRED"
    ...
    def select_prj_data_type_by_cra_accession(self,cra):
        sql = "select x.data_type_name from prj_data_type x WHERE x.data_type_id IN   (select d.data_type_id from pro_data_type d WHERE d.prj_id =  (select prj_id from cra WHERE cra.accession = (%s)));" #.format(country.strip().strip("'").strip())
        self.excute(sql,cra)
        res = self.fetchAll()
    ...
    def select_samples_with_attrs_by_cra_accession(self,cra):
        ...
        logging.debug("select_samples_with_attrs_by_cra_accession:{}".format(samples_list))
        ...
        if samples_list == None:
            ...
            for k in sample_ids:
                logging.debug("sample_process_dict[k]:{}".format(sample_process_dict[k]['sample_type_id']))
                sample_type_table_name = sample_type_mapping[str(sample_process_dict[k]['sample_type_id'])]
                
                # query attrs in sample_attr_tables
                sql2 = "select * from  {} as t  WHERE t.sample_id = '{}';".format(sample_type_table_name,sample_process_dict[k]['sample_id'])
                self.excute(sql2)
                one_sample_attr = self.fetchOne()

                filtered_attrs = { k:v for k,v in one_sample_attr.items() if k not in ['sample_id','type','taxon_id',"attribute_id","geographic_location","latitude_longitude"] }

                sample_process_dict[k]['attrs'] = filtered_attrs
                sample_process_dict[k]['package'] = package_mapping[str(sample_process_dict[k]['sample_type_id'])]
            ...
            logging.debug("dumped samples with attr:{}".format(sample_process_dict,indent=4))
...
def generate_bioproject(target_db,prj_accn,prj_title,prj_description,prj_relevance,prj_sample_scope,prj_data_type,prj_taxon_name):
    prjaction,prjadddata,prjdata,prjxmlcontent = generate_action_tree(target_db,prj_accn)
    ...
    DesDescriptionP.appendChild( doc.createTextNode(str(prj_description).strip()))
    ...
    DesExternalLinkRUL.appendChild(doc.createTextNode("https://ngdc.cncb.ac.cn/bioproject/browse/"+str(prj_accn).strip()))
    ...
    # 这里是什么就标签是什么
    DesRelevanceValue = doc.createElement(prj_relevance)
    DesRelevanceValue.appendChild(doc.createTextNode("Yes"))
...
def generate_biosample(target_db,spuid,sample_package,sample_taxonname,sample_with_attr):
    sam_action,sam_adddata,sam_data,sam_xmlcontent = generate_action_tree(target_db,spuid)
    ...
    Organism = doc.createElement("Organism")
    OrganismName = doc.createElement("OrganismName")
    OrganismName.appendChild(doc.createTextNode(sample_taxonname['name_txt']))
    Organism.appendChild(OrganismName)
    ...
    if int(sample_with_attr['sample_type_id']) == 3:
        ...
        attr.appendChild(doc.createTextNode("missing"))
        Attributes.appendChild(attr)
    elif int(sample_with_attr['sample_type_id']) in range(0,12):
        attr = doc.createElement("Attribute")
        attr.setAttribute("attribute_name","geo_loc_name")
        attr.appendChild(doc.createTextNode("missing"))
        Attributes.appendChild(attr)
...
def generate_sra(target_db,projectid,CRR_data):
    ...
    SPUID = doc.createElement("SPUID")   
    SPUID.setAttribute("spuid_namespace","NGDC")
    SPUID.appendChild(doc.createTextNode(CRR_data['run_acc']+": "+CRR_data['run_alias']))
    Identifier.appendChild(SPUID)
    adddata.appendChild(Identifier)
...
def generate_by_craacc(cra_acc):
    ...
    Description = generate_description(cra_acc, project['release_time'].strftime("%Y-%m-%d"),user["first_name"],user["last_name"],user["email"],user["organization"])
    ...
    fp = open("/workspace/project/GSA/GSA_XML/{}".format(out_name), 'w')
