...
GSA_process_path = UserPackage("GSA_process_path")
...
def makeSubCRA(craid):
    s = str(craid)
    l = 6
    sl = len(s)
    add0 = l - sl
    return "subCRA" + "0"*add0 + s


def autoGSAReport(craid, email=None):
    try:
        dbutil = mysqlUtils()
        ...
        FROM
            run_data_file 
        WHERE
            cra_id ={} and ( run_data_file.`status` = 3 OR run_data_file.`status` = 11 OR run_data_file.process_status = 32 )  and  (run_data_file.`status` != 5 )
        ORDER BY
            cra_id DESC 
        ) AS a
        ...
        if len(res) == 0:
            return {"status": "error", "data": "请核查数据是正在运行或未审核或删除！"}

        record = OrderedDict()
        # print(res)
        for x in res:
            if x['cra_id'] not in record.keys():
                ...
                statusKey = str(x['status']) + "_" + str(x['process_status'])
                if statusKey not in record[x['cra_id']]["status"].keys():
                    record[x['cra_id']]["status"][statusKey] = {
                        "run_file_name": [
                            {
                                "file": x['run_file_name'],
                                "archive_dir":x["archived_file_dir"],
                    ...
                    record[x['cra_id']]["status"][statusKey]["run_file_name"].append({
                        "file": x['run_file_name'],
                        "archive_dir": x["archived_file_dir"],
                        "run_id": x["run_id"]
                    })
        ...
        if len(record.values()) == 0:
            raise Exception(
                f"can not found any status code [{ ', '.join( list(config.error_messages.keys())) }] for {makeSubCRA(craid)}.")

        for x in record.values():

            msg = MIMEMultipart('alternative')

            sendimagefile = open('./GSA-邮件反馈.png', "rb")
            ...
            for k, m in x["status"].items():
                ...
                print(f"Found status: {k} for {makeSubCRA(craid)}")
                ...
                if k.strip().startswith("3"):
                    ...
                    for runid, archivedir in run_id_dict.items():
                        ...
                        try:
                            # res = sshExeCMD(cmd)
                            res = ftputilis.exec(cmd)
                            if len(res) == 0:
                                # res = sshExeCMD(cmd2)
                                res = ftputilis.exec(cmd2)
                            # print(res)

                            cleanres = CRRDIR.split(
                                "/")[2] + " : " + str(res).replace("/p300/gsafileprocess/temp"+CRRDIR, "").replace("/gsainsdc2/gsafileprocess/temp"+CRRDIR, "").strip()
                            # print(cleanres)
                            the_msg += "\n" + cleanres
                        except Exception as e:
            ...
            <html>
                ...
                <body>
                    ...
                    <br/>
                    <img src="cid:image" width="1300" height="900"></img>
                    <br/> -----------------------------------------------------------------------------------------------------<br/> <br/> <br/> 
                    祝好，<br/> 
                    GSA-工作组
                    </div>
            ...
            auto_mail(msg, "GSA {}提交问题反馈".format(
                makeSubCRA(x['cra_id'])), email)

            ftputilis.close()

            # print("\n".join(total_msg))
            # print(makeSubCRA(craid=craid))
            print({"status": "ok", "data": {"craid": makeSubCRA(craid=craid)}})
        return {"status": "ok", "data": {"craid": makeSubCRA(craid=craid)}}
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # userinfo = UserPackage("gsadb")
    # print(userinfo)
    autoGSAReport(19805, "sunyanling@big.ac.cn")
